...
from koi.translators import escape_xml
from koi.base_logging import mainlog

HORSE_TEMPLATE = "preorder_letter.docx"
HORSE_REFERENCE = "preorder_letter"
HORSE_TITLE = _("Preorder letter template")
...
def print_preorder_report(order_id):
    ...
    if not order.preorder_label:
        raise Exception(_("Can't print a preorder report for an order that is/was not a preorder"))
    ...
    with localcontext() as ctx: # decimal context
        ...
        for part in order.parts:
            ...
            parts_table.append( {
                ...
                'description' : escape_xml(part.description or ""),
                'quantity' : part.qty,
    ...
    try:
        ...
        raise Exception( _("Unable to open the report template (reference : '{}'). You should check in the document templates : does it exist ? Is it a .docx document ?").format(HORSE_REFERENCE))

    tpl.render(context)
    doc_path = make_home_file("preorder_letter_{}.docx".format(order.preorder_label))
