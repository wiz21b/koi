...
from sqlalchemy.orm import with_polymorphic,lazyload
...
from Task import TaskPointage
import rpctools

from chrono import *

from db_mapping import TaskOnNonBillable,TaskOnOperation,TaskOnOrder,Task,TaskActionReport,TaskOnOperation, Operation, Order, OperationDefinition,TaskActionReportType, OrderPart, ProductionFile, Customer, OrderStatusType

from db_mapping import session
...
class ClockServer(object):
    ...
    def _recordTaskOnNonBillable(self,operation_definition_id):
        ...
        q = self.dao.session.query(OperationDefinition.operation_definition_id,
    ...
    def _recordTaskOnOrder(self,order_id,opdef_id):
        ...
        q = self.dao.session.query(OperationDefinition.description,
        ...
        for o in self.dao.session.query(OperationDefinition).all():
            mainlog.debug(o.operation_definition_id)
            mainlog.debug(o)

        for o in self.dao.session.query(Order).all():
            mainlog.debug(o)
    ...
    def _recordTaskOnOperation(self,operation_id):
        ...
        q = self.dao.session.query(Operation.operation_id,
